...
// 内置 429 退避重试（智谱免费/低档 RPM 严格，必须等）
async function askViaZhipu(tile, prompt, maxRetry = 4) {
  ...
  for (let attempt = 0; attempt <= maxRetry; attempt++) {
    ...
    if (resp.status === 429 && attempt < maxRetry) {
      ...
      // 指数退避：2s, 4s, 8s, 16s（或服务端要求的 Retry-After，取较大者）
      const wait = Math.max(retryAfter * 1000, 2000 * Math.pow(2, attempt));
      console.warn(`[Zhipu 429] 第 ${attempt + 1}/${maxRetry} 次重试，等待 ${wait}ms`);
